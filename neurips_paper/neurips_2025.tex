...
% Being able to varying a system quickly while staying close to equilibrium (i.e. to the current stationary distribution) is of interest both in classical and quantum physics, and various works \cite{crooks1998nonequilibrium} have proposed adding a driving term to the physical dynamics which allow faster variation of a system's parameters. This is often referred to as \emph{counterdiabatic} driving. 
...
% Annealing \cite{crooks1998nonequilibrium} and Sequential Monte Carlo (SMC) \cite{crooks1998nonequilibrium} are examples of such approaches. 
...
We now note that when $\partial_\lambda H = \{A, H\}$, we have that $\langle \partial_\lambda H \rangle = \langle \{A, H\}\rangle = \int \rho \{A, H\} dz  = \int A \{\rho, H\} = 0$, where the penultimate step follows by an above property of the Poisson bracket. This means that $\mathbb{E}[G(z)] = \mathbb{E}[\{A, H\}^2 + (\partial_\lambda H)^2 - 2\{A, H\}(\partial_\lambda H)] = \mathbb{E}[|\{A, H\}(q,p) - \partial_\lambda H(q,p)|^2] := \mathcal{L}_H(A)$.

% But we can see that a minimum of the following:
% $R(q,p) = \langle |\{A, H\}(q,p) - \partial_\lambda H(q,p)|^2 \rangle = \langle \{A, H\}(q,p)^2 -2\{A, H\}(q,p)\partial_\lambda H(q,p) + (\partial_\lambda H(q,p))^2 \rangle$

\begin{algorithm}[t]
    \caption{Fit adiabatic gauge potential $A_\phi$ at a given $\lambda$}
    \label{alg:fit-agp}
    \begin{algorithmic}[1]
    \Require Real-valued function $A_\phi(q,p;\lambda)$, optimizer (e.g. Adam)\;, schedule $\lambda_i$, population of particles $\{q_i,p_i\}_{i=1}^N$ and weights $w_{i=1}^N$, and $T$ iterations.
    \For{$t=1,\dots,T$}
    %   \State Sample $q \sim \pi_\lambda(q) \propto e^{-U(q;\lambda)}$\; and $p \sim \mathcal{N}(0,I)$
    % = \mathbb{E}[r^2(q,p,\lambda;\phi)]
      \State Compute $l  \gets \sum_{i=1}^N w_i \left(\{A_\phi,H_\lambda\}(q_i,p_i) - \partial_\lambda V(q_i;\lambda)\right)^2$
...
\begin{algorithm}[t]
    ...
    \label{alg:cd-hmc}
    \begin{algorithmic}[1]
    \Require Population $\{q_i^{(0)}\}_{i=1}^N$ from initial distribution\;; potential $U(q;\lambda)$ and gradients\;; step size $\varepsilon$\;; schedule $\{\lambda_k\}_{k=1}^L$ with $\lambda_1=0$ (easy) and $\lambda_L=1$ (target)\;.
    ...
    \For{$k=1$ to $L-1$}
      \For{$i=1$ to $N$}
        ...
        \State \textbf{Half-step drift} (kinetic \& CD-$p$ flow):
        \[
          q_{\text{half}} \leftarrow q + \tfrac{\varepsilon}{2}\Big(p + \dot{\lambda}_k\, \nabla_p A_\phi(q,p;\lambda_k)\Big)
        \]
        \State \textbf{Full-step kick} (potential \& CD-$q$ force): 
        \[
          p \leftarrow p - \varepsilon\Big(\nabla_q U(q_{\text{half}};\lambda_{k+1}) + \dot{\lambda}_{k+1}\,\nabla_q A_\phi(q_{\text{half}},p;\lambda_{k+1})\Big)
        \]
        \State \textbf{Half-step drift} (kinetic \& CD-$p$ flow):
        \[
          q \leftarrow q_{\text{half}} + \tfrac{\varepsilon}{2}\Big(p + \dot{\lambda}_k\, \nabla_p A_\phi(q_{\text{half}},p;\lambda_k)\Big)
        \]
        \State \textbf{Protocol work increment} (discrete $\partial_t H + \nabla H \cdot v$):
        \[
          W_i \leftarrow W_i + \Big(U(q;\lambda_{k+1}) - U(q;\lambda_k)\Big) 
          + \dot{\lambda}_k \nabla H(q,p;\lambda_k) \cdot \Big(\nabla_p A_\phi(q,p;\lambda_k), -\nabla_q A_\phi(q,p;\lambda_k)\Big)
        \]
      ...
      \State Learn $A_{\lambda_{k+1}}$ using the weighted population (Algorithm 1)
    \EndFor
...
This means that we can minimize the loss $\mathcal{L}_H(A)$ instead, which is simple to calculate.

% $\mathbb{E}[R(A, H)] = \mathbb{E}[|\{A, H\} - \partial_\lambda H|^2 ]\rangle$ instead, which is simple to calculate.
...
% In practice, Hamiltonian Monte Carlo uses discretized Hamiltonian dynamics as a kernel, and accounts for the bias induced by discretization by the Metropolis-Hastings algorithm \cite{crooks1998nonequilibrium}. To ensure ergodicity, momentum can be refreshed every $n$ steps, or partially refreshed every step, which corresponds to the discretization of underdamped Langevin dynamics \cite{crooks1998nonequilibrium}. 
...
When $A$ is learned perfectly, and the discretization step size is taken to zero, transport is perfect. However, in practice, we must use a finite step size, and we must approximate $A$. This introduces discrepancy between the distribution of the particles at time $t$ and $\rho_{H(t)}$, which we can correct for with weights if necessary.

When a weighting scheme is used, our proposed algorithm is a special case of the general framework of Sequential Monte Carlo samplers \cite{doucet2001sequential}, as shown in appendix \ref{app:smc}.

Crucially, the estimate of the expectation $\mathbb{E}[R(A, H)]$ can be calculated using the weighted particles, which improves the learning of $A$.
...
This can be understood as a special case of the general framework of Sequential Monte Carlo samplers \cite{doucet2001sequential}, as shown in appendix \ref{app:smc}.
...
\section{Counterdiabatic HMC as an SMC sampler}
\label{app:smc}

% \paragraph{Path space and a proposal law.}
% Let \((\mathsf X,\mathcal B)\) be a measurable state space and fix a horizon \(T\in\mathbb N\).
% Write \(x_{0:T}=(x_0,\ldots,x_T)\in \mathsf X^{T+1}\) and endow the path space
% \((\mathsf X^{T+1},\mathcal B^{\otimes (T+1)})\) with the product $\sigma$-algebra.


Given an initial distribution \(\mu\in\mathcal P(\mathsf X)\) and a sequence of Markov kernels
\(M_t\in\mathcal M(\mathsf X)\) for \(t=1,\ldots,T\), the \emph{proposal} (or reference) path
measure \(\mathbb Q\) is
\begin{equation}
\label{eq:Q-path}
\mathbb Q(\mathrm d x_{0:T})
=\mu(\mathrm d x_0)\,\prod_{t=1}^T M_t(x_{t-1},\mathrm d x_t).
\end{equation}

\paragraph{Importance sampling on path space (Feynman--Kac).}
Let \(G_0:\mathsf X\to(0,\infty)\) and \(G_t:\mathsf X\times\mathsf X\to(0,\infty)\) for \(t\ge1\)
be strictly positive \emph{potentials}. The \emph{target} path measure \(\mathbb P\) is defined by
reweighting \(\mathbb Q\):
\begin{equation}
\label{eq:FK-path}
\mathbb P(\mathrm d x_{0:T})
=\frac{1}{Z}\,G_0(x_0)\Bigg[\prod_{t=1}^T G_t(x_{t-1},x_t)\Bigg]\,\mathbb Q(\mathrm d x_{0:T}),
\end{equation}
where the normalizing constant is
\(Z=\mathbb E_{\mathbb Q}\!\left[G_0(X_0)\prod_{t=1}^T G_t(X_{t-1},X_t)\right]\).
The resulting unnormalized \emph{marginal} measures \(\gamma_t\in\mathcal S(\mathsf X)\) and their
normalized counterparts \(\eta_t\in\mathcal P(\mathsf X)\) are
\begin{align}
\label{eq:gamma-eta}
\gamma_t(\varphi)&=
\mathbb E_{\mathbb Q}\!\left[\varphi(X_t)\,G_0(X_0)\prod_{s=1}^t G_s(X_{s-1},X_s)\right],\qquad
Z_t:=\gamma_t(1),\\
\eta_t(\varphi)&=\gamma_t(\varphi)/Z_t,\qquad t=0,\ldots,T .
\end{align}
Eqs.~\eqref{eq:Q-path}--\eqref{eq:gamma-eta} constitute the standard Feynman--Kac framework:
importance sampling is performed on the \emph{path} measure, while particle approximations target
the time marginals \((\eta_t)_{t=0:T}\).

One can then sample from todo
by initializing a population of particles from $\pi_0$ and then propagating them forward in time using the kernel $M_t$, keeping track of the weights at each step and optionally resampling to mitigate weight degeneracy.
...
\paragraph{Sequential Monte Carlo samplers}

\cite{doucet2001sequential} show how the general framework of Sequential Monte Carlo as summarized above can be applied to the problem of sampling from a distribution $\pi$ on $\mathbb{R}^d$, by artificially constructing a path of distributions $\pi_t$ with $\pi_0$ a simple distribution and $\pi_T = \pi$ the target distribution.
...
% A particularly useful specification is obtained by introducing, for each \(t\ge1\), a
% \emph{backward} Markov kernel \(L_{t-1}\in\mathcal M(\mathsf X)\).
% Assuming absolute continuity,
% define the potentials by the Radon--Nikodym derivative
% \begin{equation}
% \label{eq:G-rn}
% G_t(x_{t-1},x_t)=
% \frac{\mathrm d\!\left(L_{t-1}\otimes \gamma_t\right)}
%      {\mathrm d\!\left(\gamma_{t-1}\otimes M_t\right)}(x_{t-1},x_t),
% \qquad t=1,\ldots,T,
% \end{equation}
% with \(G_0\equiv 1\). Here \( \alpha\otimes K\) denotes the measure on
% \(\mathsf X\times\mathsf X\) given by \(\alpha(\mathrm d x)\,K(x,\mathrm d y)\).
% Under \eqref{eq:G-rn}, the path measure \eqref{eq:FK-path} indeed has marginals \((\eta_t)\),
% and the incremental importance weight at time \(t\) depends only on \((x_{t-1},x_t)\).

% A common and convenient choice is to take \(M_t\) to be \(\eta_t\)-invariant
% (\(\eta_t M_t=\eta_t\)) and \(L_{t-1}\) its time reversal:
% \begin{equation}
% \label{eq:time-reversal}
% L_{t-1}(x_t,\mathrm d x_{t-1})
% =\frac{\eta_{t-1}(\mathrm d x_{t-1})\,M_t(x_{t-1},\mathrm d x_t)}{\eta_t(\mathrm d x_t)}.
% \end{equation}
% Plugging \eqref{eq:time-reversal} into \eqref{eq:G-rn} yields the simple potential
% \begin{equation}
% \label{eq:ais-G}
% G_t(x_{t-1},x_t)=\frac{\gamma_t(x_t)}{\gamma_{t-1}(x_{t-1})},
% \end{equation}
% which recovers annealed/bridging SMC and AIS.

\paragraph{SMC with physical dynamics: nonequilibrium fluctuation theorems}

In the case where $M$ and $L$ are given by forward time and backward time Hamiltonian dynamics (or overdamped or underdamped Langevin dynamics), the ratio of $M(x_t | x_{t-1})$ to $L(x_{t-1} | x_t)$ can be calculated using the non-equilibrium fluctuation theorem derived by \cite{crooks1998nonequilibrium} and extended to more generality \cite{todo}.

First, recall that

$$
\frac{dH}{dt} = \frac{\partial H}{\partial t} + \nabla H \cdot \frac{d\mathbf{x}}{dt} := dW + dQ
$$
...
where $W$ is the integral of $dw = \frac{\partial H}{\partial t}$ over the trajectory, and $\Delta F = -\log\frac{Z_t}{Z_{t-1}}$, for $Z_t := \int \pi_{H(t)}(z)dz$.

The consequence is that we can compute 
...
Since Hamiltonian dynamics are measure-preserving, i.e. $Q=0$, even when numerically simulated with a leapfrog integrator, we see immediately that when using an unadjusted HMC kernel in the SMC sampling algorithm, the correct weight is given by $e^{-\Delta H}$, in agreement with Appendix B of \cite{dai2022invitation}.

The more interesting case concerns a kernel $k$ that arises from a counterdiabatic Hamiltonian $H_c$. In this case, \cite{zhong2024time} derives an analogous theorem, but in terms of a different work, which we refer to as $W_c$, obtained by integrating $dw_c = todo$ over the trajectory, where

$$
dw_c = (\frac{\partial H_c}{\partial t} - \nabla H\cdot v_A + \nabla \cdot v_A)dt
$$

where $v_A$ is the drift induced by $A$, i.e. $v_A = (\nabla_q A, -\nabla_p A)$. We then see that the third term vanishes, since Hamiltonian flow is dissipationless. In this case, we see that the physical intepretation of heat and work is preserved, in the sense that 

$$
\frac{dH}{dt} = \frac{\partial H}{\partial t} + \nabla H \cdot v + \nabla H \cdot v_H
$$

where $v_H$ is the drift induced by $H$, i.e. $v_H = (\nabla_q H, -\nabla_p H)$. This means that $dw_c + dq = dH$, where $\nabla H \cdot v_H$ is defined as before.
