...
def run_naive_hmc_simulation(M, N_steps, delta_t, momentum_refresh_interval, make_T, make_V, lam_fn, dot_lam_fn, key, dim, use_weights=False, ess_threshold=0.5, snapshot_interval=10):
    ...
    for k in range(N_steps + 1):
        ...
        jax.debug.print("\nt_k: {x}\n", x=(t_k, lam_k))
        jax.debug.print("\n\nmean and variance of q_naive\n\n: {x}", x=(jnp.mean(q_naive), jnp.var(q_naive)))
        ...
        if use_weights:
            ...
            # do leapfrog manually
            # p_naive = jax.random.normal(jax.random.PRNGKey(0), (M, dim))
            # p_half = p_naive - (0.01/2)*jax.grad(make_T(0))(p_naive)
            # q_naive = q_naive + 0.01*jax.grad(make_V(0))(q_naive)
            # p_naive = p_half - (0.01/2)*jax.grad(make_V(0))(q_naive)
